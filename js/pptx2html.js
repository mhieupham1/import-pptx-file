...
Reveal.initialize({\
	...
	$("#result").bind('DOMNodeInserted', debounce(() => {
		const elementResult = $("#result").children('section');
		console.log(elementResult);
		for (let i = 0; i < elementResult.length;i ++) {
			console.log(elementResult[i]);
			domtoimage.toPng(elementResult[i])
				.then(function (dataUrl) {
					fabric.Image.fromURL(dataUrl, function(img) {
						var oImg = img.set({ left: 1600/2, top: 900/2}).scale(1);
						canvas.add(oImg);
					});
				})
